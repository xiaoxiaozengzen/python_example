...
def using_av_another(image_path: str, output_path: str):
    container = av.open(output_path, mode='w')
    ...
    # codec_name：字符串类型，编码器类型：h264, h265, mpeg4等
    # mode：打开模式，'w'表示编码，‘r’表示解码
    codec = av.CodecContext.create('h264', 'w')
    codec.width = img.width
    codec.height = img.height
    codec.pix_fmt = 'yuv420p'  # 设置像素格式为 YUV 4:2:0 JPEG格式
    codec.time_base = av.time_base  # 默认时间基准，通常为1/1000000秒
    codec.bit_rate = 1024 * 1024 * 8  # 编码目标码率，编码器会尽量控制输出码率在这个范围内
    # codec.encode(frame)/codec.decode(packet)，都返回list
    ...
    while True:
        ...
        img = Image.open(image_file)
        if img.size != (codec.width, codec.height):
            img = img.resize((codec.width, codec.height))
        ...
        # packet：是编码后的数据块，例如压缩后的码流片段(h264, h265等)
        # 特点：已压缩，适合传输和存储，不能直接处理
        for packet in codec.encode(video_frame):
            packet_count += 1
            print(f"============ frame: {frame_count} packet {packet_count}============")
            print("pts: ", packet.pts)
            print("dts: ", packet.dts)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python video2image.py <image_path> <output_path> <method>")
    else:
        ...
        if sys.argv[3] == "1":
            print("Using av method 1")
            using_av(image_path, output_path)
        elif sys.argv[3] == "2":
            using_av_another(image_path, output_path)